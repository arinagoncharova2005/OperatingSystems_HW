...
void read_file(FILE* file){
    ...
    while (fgets(str, sizeof(str), file) != NULL) {
            ...
            ProcessData process = {.idx = idx, .at = at, .bt = bt, .rt = 0, .wt = 0, .ct = 0, .tat = 0, .burst = bt};
            data[i] = process;
            data_size++;
            i++;
    ...
    for (int j = 0; j < sizeof(ps)/sizeof(pid_t); j++) {
        ps[j] = 0;
    }
}
...
ProcessData find_next_process() {
    ...
    //if the process burst time has not expired
    if (data[running_process].burst > 0 ){
        return data[running_process];
    }

    int arrive_time_cur = total_time;
    if (running_process != -1) {
        arrive_time_cur = data[running_process].at;
    }

    // TODO: find location of the next process to run from the {data} array
    int arrive_time_next = INT_MAX;

    for(int i=0; i < data_size; i++) {
        if (data[i].at >= arrive_time_cur && data[i].burst != 0) {
            if (data[i].at < arrive_time_next) {
                arrive_time_next = data[i].at;
                location = data[i].idx;
                //printf("Location %d\n", location);
            } else if (data[i].at == arrive_time_next) {
                if (data[i].idx < location) {
                    location =  data[i].idx;
                }
